...
Run: python3 -m pytest -q tests/runtime/wiki/test_scenarios_content.py
"""
...
# The runtime under test lives in runtime/wiki/; these tests live in tests/runtime/wiki/.
HERE = Path(__file__).resolve().parents[3] / "runtime" / "wiki"
CI = "docs-site ci"
...
def deploy(repo, wiki_bare, **extra):
    env = dict(os.environ, WIKI_URL=str(wiki_bare), REPO_PUSH_URL="origin", CI_PROJECT_PATH="group/docs-site",
