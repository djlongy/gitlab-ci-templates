...
Run: python3 -m pytest -q tests/runtime/wiki/test_scenarios_races.py

Every test drives runtime/wiki/wiki-deploy.sh end to end (subprocess) the way CI does, against:
...
def deploy(repo, wiki_bare, **extra):
    ...
    # CI_SERVER_FQDN=example.com makes the script's sync identity "docs-site ci <ci@example.com>",
    # the identity the fixture's first sync commit carries, so a no-op run is recognised as one.
    env.update(WIKI_URL=str(wiki_bare), REPO_PUSH_URL="origin", CI_PROJECT_PATH="group/docs-site",
